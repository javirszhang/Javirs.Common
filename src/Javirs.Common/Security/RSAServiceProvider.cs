...
namespace Javirs.Common.Security
{
    ...
    /// RSA加密服务提供类
    /// </summary>
    public class RSAServiceProvider : IRsa
    {
        /// <summary>
        /// .NET内置RSA加解密对象
        /// </summary>
        ...
        /// <summary>
        /// 使用RSA私钥对数据进行签名，PKCS#1签名
        /// </summary>
        /// <param name="buffer"></param>
        /// <returns></returns>
        public string SignData(byte[] buffer)
        {
            ...
            //return Convert.ToBase64String(rsf.CreateSignature(hv));
            return SignData(buffer, "MD5");
        }
        public string SignData(byte[] buffer, string algorithm)
        {
            byte[] hv = null;
        ...
        public static byte[] RSA(byte[] data, byte[] E, byte[] M)
        {
            ...
            else
            {
                ...
                var resInteger = cryptoData.modPow(e, n);
                resBytes.AddRange(resInteger.getBytes());
            }
        ...
        public static byte[] AddPKCS1Padding(byte[] buffer, int keysize, bool isPublicKey)
        {
            ...
            if (buffer.Length <= keysize - 11)
            {
                ...
                {
                    res[i] = flag == 0x02 ? (byte)(new Random().Next(1, 0xFF)) : (byte)0xFF;
                }
                return res;
            }
            else
